...
>
> **Última atualização:** 25/09/2026 (2ª leva do dia)
...
- **Line** (`product_lines`): name (unique), active, **luxo** (flag de linha de luxo), sortOrder. Cadastro de linha de produto (Kids, Pet, Inverno, Verão…). Segue o modelo da Category — hoje é só o cadastro; os pontos de uso (incl. vínculo com produto) virão em feature seguinte.
...
- **Linhas:** CRUD sob `/api/admin/lines` (nome único, `active`, **`luxo`**, `sortOrder`). Sem regra de exclusão por produto ainda (o `Product` não tem campo `line`); será plugada quando os pontos de uso forem criados.
...
- **Linhas de produto:** cadastro central (Cadastros → Linhas) de linhas (ex.: Kids, Pet, Inverno, Verão) com nome único, ativo/inativo, ordem e a flag **Luxo**. Nesta fase é só o cadastro (CRUD `/api/admin/lines`); os pontos do sistema que consomem a linha serão adicionados numa feature seguinte.
...
> As 3 branches estão alinhadas em `87b1ab3`. Últimas levas: **data de nascimento + notificações de aniversário** (`e8c764f`), **backfill de pagamento p/ vendas antigas** (`79d8e58`), **Dashboard Financeiro** (`d98827a`) e **cadastro de linhas de produto com flag luxo** (`87b1ab3`).
