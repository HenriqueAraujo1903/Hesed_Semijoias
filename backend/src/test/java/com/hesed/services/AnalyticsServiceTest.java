...
class AnalyticsServiceTest {
    ...
    void stock_buildsPayload() {
        ...
        when(productRepository.stockKpis()).thenReturn(List.<Object[]>of(
                new Object[]{5L, 42L, new BigDecimal("1000.00"), new BigDecimal("3000.00")}));
        when(productRepository.stockCountByStatus()).thenReturn(List.of(
                new Object[]{"DISPONIVEL", 3L},
                new Object[]{"BAIXO", 1L},
                new Object[]{"ESGOTADO", 1L}));
        when(productRepository.stockByCategory()).thenReturn(List.of(
        ...
        when(productRepository.findCriticalStock()).thenReturn(List.of(out, low));
        ...
        when(stockMovementRepository.findRecentWithProduct(any())).thenReturn(List.of(mov));

        StockAnalyticsResponse r = service.stock();
    ...
    void stock_empty() {
        when(productRepository.stockKpis()).thenReturn(List.of());
        when(productRepository.stockCountByStatus()).thenReturn(List.of());
        when(productRepository.stockByCategory()).thenReturn(List.of());
        when(productRepository.findCriticalStock()).thenReturn(List.of());
        when(stockMovementRepository.findRecentWithProduct(any())).thenReturn(List.of());
