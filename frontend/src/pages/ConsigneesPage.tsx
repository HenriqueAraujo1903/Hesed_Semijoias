...
export default function ConsigneesPage() {
  ...
  const [editTarget, setEditTarget] = useState<Consignee | null>(null);
  ...
  function openEdit(c: Consignee) { setEditTarget(c); setShowForm(true); }

  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-stone-800">Revendedoras</h1>
          <p className="mt-1 text-sm text-stone-500">Gerencie as parceiras e suas comissões.</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border-2 border-dashed border-stone-200 bg-white py-16 text-center">
          <p className="text-sm text-stone-500">Nenhuma revendedora cadastrada.</p>
        </div>
      ) : (
        <>
          {/* Tabela (desktop) */}
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-stone-100 text-sm">
                <thead className="bg-stone-50">
                  <tr>
                    {['Nome', 'Telefone', 'E-mail', 'Comissão', 'Ações'].map((col) => (
                ...
                <tbody className="divide-y divide-stone-50">
                  {consignees.map((c) => (
                    <tr key={c.id} className="hover:bg-stone-50 transition-colors">
                      ...
                      <td className="px-4 py-3 font-medium text-stone-700">{PCT.format(c.commissionRate)}</td>
            ...
            {consignees.map((c) => (
              <div key={c.id} className="rounded-2xl border border-stone-200 bg-white p-4 shadow-sm">
                <div className="flex items-start justify-between gap-3">
                  <p className="font-medium text-stone-800 min-w-0 flex-1 truncate">{c.name}</p>
                  <span className="shrink-0 rounded-full bg-gold-50 px-2 py-0.5 text-[10px] font-medium text-gold-700">
                    {PCT.format(c.commissionRate)}
                  </span>
                </div>
...
function ConsigneeModal({ consignee, onClose, onSaved }: {
  consignee: Consignee | null; onClose: () => void; onSaved: () => void;
}) {
  const isEdit = consignee !== null;
  const [form, setForm] = useState({
    name: consignee?.name ?? '',
    phone: consignee?.phone ?? '',
    email: consignee?.email ?? '',
    commissionRate: consignee ? (consignee.commissionRate * 100).toString() : '',
  });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (isEdit) {
        await api.put(`/consignees/${consignee.id}`, body);
      } else {
      ...
      <div className="w-full max-w-md rounded-2xl bg-white shadow-xl">
        <div className="flex items-center justify-between border-b border-stone-100 px-6 py-4">
          <h2 className="text-base font-semibold text-stone-800">
            {isEdit ? 'Editar Revendedora' : 'Nova Revendedora'}
          </h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 px-6 py-5">
          ...
          <div>
            <label className="block text-xs font-medium text-stone-600 mb-1">Comissão (%) *</label>
            <input required type="number" step="0.5" min="0" max="100" value={form.commissionRate}
